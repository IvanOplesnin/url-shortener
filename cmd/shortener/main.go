...
func createRepo(cfg *config.Config) (*persisted.Repo, *pgxpool.Pool, error) {
	...
	if cfg.DBDSN != "" {
		...
		persisterdRepo, err := persisted.New(repo, nil, nil, fileStorage, nil, repo, repo, repo, repo)
		if err != nil {
			return nil, nil, err
		}
	...
	persisterdRepo, err := persisted.New(repo, repo, repo, fileStorage, repo, nil, repo, repo, repo)
	if err != nil {
		return nil, nil, err
	}
